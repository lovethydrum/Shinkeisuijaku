...
class Cards:
    ...
    def flip_card(self):
        ...
        if self.letter.isupper():
            ...
            game_window.blit(reveal_letter, (self.x+(32*resize_factor), self.y + (30*resize_factor)))
        else:
            reveal_letter = font.render(self.letter, True, (0, 0, 0), )
            game_window.blit(reveal_letter, (self.x + (45*resize_factor), self.y + (40*resize_factor)))
        global flip_count
        flip_count += 1
...
while game_running:
    ...
    if game_state == "range: start":
        ...
        game_window.blit(one_text, (342*resize_factor, 450*resize_factor))
        game_window.blit(two_text, (638*resize_factor, 450*resize_factor))
        game_window.blit(three_text, (923*resize_factor, 450*resize_factor))
        game_window.blit(four_text, (1232*resize_factor, 450*resize_factor))
        for items in selection_list:
            items.draw_card()
        for event in ev:
            if event.type == pygame.MOUSEBUTTONDOWN:
